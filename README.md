...
This project automatically scrapes cloud provider case studies, processes them, and stores them in a database for further analysis and embedding. Currently supports AWS and Google Cloud Platform (GCP).

## Features

- Automatically scrapes case studies from cloud provider websites
- Organizes data by provider (AWS, GCP, etc.)
...
- Includes a scheduler for automated daily runs
...
2. For detailed deployment instructions to Google Cloud Run, see [CLOUD_DEPLOYMENT.md](CLOUD_DEPLOYMENT.md).
...
- `POST /scrape`: Trigger scraping process
  - Query parameters:
    ...
    - `max_pages`: Maximum page number to scrape (default: 3 for AWS, 2 for GCP)
...
Both AWS and GCP implementations follow the same workflow pattern:

1. **Link Extraction**:
   - Links are scraped from provider websites
   - Each link is stored in a CSV file in the backup directory (`backup/aws/aws_case_study_links.csv` or `backup/gcp/gcp_case_study_links.csv`)
   - The CSV includes:
...
2. **Case Study Processing**:
   ...
   - For each case study:
     - Downloads the case study as a PDF to the provider's PDF directory (`AWS/PDF/` or `GCP/PDF/`)
     - Saves the link as a text file in the provider's TXT directory (`AWS/TXT/` or `GCP/TXT/`)
...
This consistent workflow ensures that both AWS and GCP case studies are processed in the same way, making the system easier to maintain and extend.
...
| `python scheduler.py --scrape` | Run immediate scraping for all providers. This scrapes case studies from all configured providers (AWS, GCP) using default settings. |
...
- `id`: Unique identifier
- `provider`: Cloud provider (aws, gcp, etc.)
- `link`: Original URL
- `pdf_path`: Path to the saved PDF file
- `txt_path`: Path to the text file containing the link
- `content`: Extracted text content
